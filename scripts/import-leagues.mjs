...
async function fetchCompetitions({ countryId, federationId, page = 1, size = 200 }) {
  ...
  if (
    json?.success !== false &&
    Array.isArray(competitionsPrimary) &&
    competitionsPrimary.length === 0
  ) {
    const urlFallback = `${base}/competitions/listing.json?${qs.toString()}`
    console.log(
      '[FALLBACK] Пустой список от /competitions/list.json, пробуем /competitions/listing.json',
    )
...
function toLeagueDoc(competition) {
  ...
  return {
    ...
    countries: (competition.countries || []).map((c) => ({
      id: Number(c.id),
      name: translateCountryName(c.name),
    })),
...
async function upsertLeague(payload, data) {
  ...
  if (payloadData.season?.name) {
    console.log(
      `         • season: "${payloadData.season.name}" (${payloadData.season.start || 'н/д'} - ${payloadData.season.end || 'н/д'})`,
    )
  }
  ...
  if (existing.docs.length > 0) {
    ...
    try {
      ...
      if (doc.displayName !== payloadData.displayName)
        changes.push(`displayName: "${doc.displayName}" → "${payloadData.displayName}"`)
      if (doc.countryId !== payloadData.countryId)
        changes.push(`countryId: ${doc.countryId} → ${payloadData.countryId}`)
      if (doc.countryName !== payloadData.countryName)
        changes.push(`countryName: "${doc.countryName}" → "${payloadData.countryName}"`)
      if (doc.isLeague !== payloadData.isLeague)
        changes.push(`isLeague: ${doc.isLeague} → ${payloadData.isLeague}`)
      if (doc.isCup !== payloadData.isCup)
        changes.push(`isCup: ${doc.isCup} → ${payloadData.isCup}`)
      if (doc.tier !== payloadData.tier) changes.push(`tier: ${doc.tier} → ${payloadData.tier}`)
      if (doc.hasGroups !== payloadData.hasGroups)
        changes.push(`hasGroups: ${doc.hasGroups} → ${payloadData.hasGroups}`)
      if (doc.active !== payloadData.active)
        changes.push(`active: ${doc.active} → ${payloadData.active}`)
      if (doc.nationalTeamsOnly !== payloadData.nationalTeamsOnly)
        changes.push(
          `nationalTeamsOnly: ${doc.nationalTeamsOnly} → ${payloadData.nationalTeamsOnly}`,
        )
  ...
  try {
    ...
    if (
      ...
      console.log(
        `[UPSERT] Конфликт уникальности для competitionId=${payloadData.competitionId}, повторный поиск`,
      )
      ...
      if (existing.docs.length > 0) {
        ...
        if (!targetDoc) {
          targetDoc = existing.docs.find(
            (doc) => doc.name === payloadData.name && doc.countryId === payloadData.countryId,
          )
        }
        if (!targetDoc) {
    ...
    console.error(
      `[UPSERT][CREATE] Критическая ошибка при создании записи competitionId=${payloadData.competitionId}:`,
      e.message,
    )
    if (e.data?.errors) {
...
async function discoverByFilters(payload, { pageSize = 200 } = {}) {
  ...
  for (const raw of federations) {
    ...
    for (const comp of comps) {
      ...
      console.log(
        `      ↳ ${action.toUpperCase()} ${doc.competitionId} ${doc.name} (payloadId=${id})`,
      )
      processed += 1
    }
  }
...
async function main() {
  ...
  while (true) {
    ...
    console.log(
      `Получено: ${competitions.length}, pages=${pages}, total=${totalItems}, за ${durationMs} мс`,
    )
    ...
    for (const comp of competitions) {
      ...
      console.log(
        `  • [${ordinal}${total ? `/${total}` : ''}] ${doc.competitionId} ${doc.name}${countryLabel}`,
      )
      ...
      try {
        ...
        if (processed % 25 === 0) {
          console.log(
            `[PROGRESS] Обработано ${processed}${total ? `/${total}` : ''} (создано: ${stats.created}, обновлено: ${stats.updated}, пропущено: ${stats.skipped})`,
          )
        }
      } catch (e) {
  ...
  if (processed === 0) {
    console.log(
      '[DISCOVERY] Прямой список пуст. Пробуем обход через каталоги (federations/countries)…',
    )
