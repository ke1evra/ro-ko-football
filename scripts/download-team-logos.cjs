...
function ensureDirectories() {
  const dirs = [CONFIG.LOGOS_DIR, path.dirname(CONFIG.LOG_FILE), path.dirname(CONFIG.PROGRESS_FILE)]
...
async function main() {
  ...
  log(
    `Загружен прогресс: страница ${progress.currentPage}, загружено ${progress.downloadedLogos} логотипов`,
  )
  ...
  try {
    ...
    while (true) {
      ...
      log(
        `Страница ${currentPage - 1} обработана. Всего: команд ${progress.totalTeams}, загружено ${progress.downloadedLogos}, пропущено ${progress.skippedLogos}, ошибок ${progress.failedLogos}`,
      )
