...
const CONFIG = {
  ...
  LOGOS_DIR: path.join(__dirname, '..', 'public', 'team-logos-full-test'),
  LOG_FILE: path.join(__dirname, '..', 'logs', 'team-logos-full-test.log'),
  PROGRESS_FILE: path.join(__dirname, '..', 'logs', 'team-logos-full-test-progress.json'),
  PAGE_SIZE: 10,
  DELAY_BETWEEN_REQUESTS: 500, // 1 секунда между запросами
  DELAY_BETWEEN_DOWNLOADS: 200, // 0.5 секунды между загрузками логотипов
}
