...
import type { GetAuthVerifyJsonQueryResponse, GetAuthVerifyJson400, GetAuthVerifyJson401, GetAuthVerifyJson500 } from "../../types/GetAuthVerifyJson.ts";
...
export async function getAuthVerifyJson(config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<GetAuthVerifyJsonQueryResponse, ResponseErrorConfig<GetAuthVerifyJson400 | GetAuthVerifyJson401 | GetAuthVerifyJson500>, unknown>({ method : "GET", url : getGetAuthVerifyJsonUrl().url.toString(), ... requestConfig })  
  return res
}