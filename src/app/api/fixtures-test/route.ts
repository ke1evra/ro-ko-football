...
export async function GET(req: NextRequest) {
  ...
  const testCases = [
    ...
    { name: 'current date YYYY-MM-DD', params: { date: new Date().toISOString().split('T')[0], size: 20 } },
    { name: 'tomorrow', params: { date: new Date(Date.now() + 24*60*60*1000).toISOString().split('T')[0], size: 20 } },
    { name: 'yesterday', params: { date: new Date(Date.now() - 24*60*60*1000).toISOString().split('T')[0], size: 20 } },
    { name: 'no date param', params: { size: 20 } },
    { name: 'from-to range (3 days)', params: { 
