    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          {seasonName && (
            <CardDescription>{seasonName}</CardDescription>
          )}
        </CardHeader>
        <CardContent>
              ...
              {standings.map((team, index) => (
                <TableRow key={team.team_id || index} className="hover:bg-muted/50">
                  ...
                  <TableCell className={`text-center font-medium ${
                    Number(team.goal_diff || 0) > 0 ? 'text-green-600' : 
                    Number(team.goal_diff || 0) < 0 ? 'text-red-600' : 
                    'text-muted-foreground'
                  }`}>
                    {Number(team.goal_diff || 0) > 0 ? '+' : ''}{team.goal_diff || 0}
                  </TableCell>
