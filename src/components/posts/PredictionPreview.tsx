...
export default function PredictionPreview({ prediction, className = '' }: PredictionPreviewProps) {
  ...
  return (
    <div
      className={`bg-gradient-to-r from-primary/5 to-primary/10 rounded-lg p-4 border border-primary/20 ${className}`}
    >
      {/* Информация о матче */}
      {hasMatchInfo && (
        <div className="mb-3">
        ...
        <div className="mb-3">
          ...
          <div className="flex flex-wrap gap-2">
            {events.map((event, index) => (
              ...
              >
                {event.event}
                <span className="ml-1 font-bold">{event.coefficient.toFixed(2)}</span>
              </Badge>
            ))}
          </div>
        </div>
