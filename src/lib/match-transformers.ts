 ...
 */
export function transformMatchStatsData(apiStats: LiveScoreMatchStats, matchPayloadId: string, syncSource: string = 'stats') {
  const now = new Date().toISOString()
  ...
  return {
    ...
    // События
    events: apiStats.events?.map(event => ({
      minute: event.minute,
    ...
    lineups: apiStats.lineups ? {
      home: apiStats.lineups.home ? {
        ...
        startingXI: apiStats.lineups.home.starting_xi?.map(player => ({
          number: player.number || null,
          name: player.name,
          position: player.position || null,
        })) || [],
        substitutes: apiStats.lineups.home.substitutes?.map(player => ({
          number: player.number || null,
          name: player.name,
          position: player.position || null,
        })) || [],
      ...
      away: apiStats.lineups.away ? {
        ...
        startingXI: apiStats.lineups.away.starting_xi?.map(player => ({
          number: player.number || null,
          name: player.name,
          position: player.position || null,
        })) || [],
        substitutes: apiStats.lineups.away.substitutes?.map(player => ({
          number: player.number || null,
          name: player.name,
          position: player.position || null,
        })) || [],
 ...
 */
export function getHistoricalDateRange(days: number, fromDate?: Date): { from: string; to: string } {
