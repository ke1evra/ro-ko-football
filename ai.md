...
- Безопасность ключей: серверный маршрут использует обёртку над fetch `src/lib/http/livescore/customFetch.ts` (совместимую с kubb), ключи берутся из env (LIVESCORE\_\*).
