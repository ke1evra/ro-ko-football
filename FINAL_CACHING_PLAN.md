...
### Этап 1: Логирование (2 часа) 🔴 КРИТИЧНО
...
**Изменить:**
1. `src/payload.config.ts` - добавить коллекцию `ApiRequestLogs`
2. `src/lib/http/livescore/customFetch.ts` - использовать логирование

**Результат**: 
- Каждый запрос к LiveScore записывается в БД
- Автоматическое отключение при достижении 45k запросов/день
- Алерты при 80% лимита (36k запросов)

**Документация**: После завершения → `docs/STAGE_1_LOGGING.md`
...
7. `src/collections/ApiRequestLogs.ts` (логирование)

**Изменить:**
1. `src/payload.config.ts` - добавить 7 коллекций
2. `src/collections/Matches.ts` - добавить поле `fixtureId` (если нет) и связь с `Fixtures`
...
**Ручная синхронизация (создать скрипты, запуск через админку):**
5. `scripts/sync-standings.mjs` - синхронизация таблиц 🔧
6. `scripts/sync-teams.mjs` - синхронизация команд 🔧
7. `scripts/sync-countries.mjs` - синхронизация стран 🔧
