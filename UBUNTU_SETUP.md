...
### 5.1. Настройка политики перенаправления для Docker

По умолчанию `ufw` блокирует трафик, который Docker пытается перенаправить. Это необходимо исправить для корректной работы сети.

```bash
# Разрешаем ufw перенаправлять пакеты
sudo sed -i 's/DEFAULT_FORWARD_POLICY="DROP"/DEFAULT_FORWARD_POLICY="ACCEPT"/' /etc/default/ufw
```

### 5.2. Добавление разрешающих правил

Теперь добавим правила для доступа к необходимым портам.
...
### 5.3. Активация и перезагрузка

```bash
# Включаем файрвол
sudo ufw enable

# Перезагружаем, чтобы применить все изменения
sudo ufw reload

# Перезапускаем Docker, чтобы он корректно применил свои сетевые правила
sudo systemctl restart docker
```

**Примечание:** Правило для порта `27017` (MongoDB) больше не нужно, так как `bindIp: 0.0.0.0` и разрешенная политика перенаправления позволяют Docker-контейнерам обращаться к базе данных.
